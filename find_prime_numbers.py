...
def get_execution_time_in_seconds(function_name, start, end):
    ...
    average_time = total_time_in_seconds / TIMEIT_NUMBER_OF_CALLS
    return average_time
...
def print_primes_from_fastest_approach(start, end, show_timeit_results):
    ...
    for function_name, _ in get_approaches().items():
        average_time = get_execution_time_in_seconds(function_name, start, end)
        time_by_function[function_name] = average_time
        if show_timeit_results:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--show_times', dest='show_timeit_results', type=bool,
                        default=DEFAULT_SHOW_TIMES,
                        help='Show the time it took for each and every approach. (default: true)')
